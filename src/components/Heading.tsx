...
const Heading = ({ size = 1, className = '', children }: PropsWithChildren<TitleProps>) => {
  ...
  if (size === 1) {
    return <h1 className={clsx(defaultClasses, 'text-2xl text-zinc-900')}>{children}</h1>
  } else if (size === 2) {
    return <h2 className={clsx(defaultClasses, 'mb-6 text-2xl text-zinc-800')}>{children}</h2>
  } else if (size === 3) {
